...
var KTDefaultDatatableDemo = function() {
    ...
    var demo = function() {
            ...
            toolbar: {
                ...
                items: {
                    ...
                    pagination: {
                        ...
                        [
                            5, 10, 20, 30, 50
                        ], // display dropdown to select pagination size. -1 is used for "ALl" option
                    },
                },
            },
        ...
        $("#kt_datatable_clear").on("click",
            function() {
                $("#kt_datatable_console").html("");
            });

        $("#kt_datatable_reload").on("click",
            function() {
                datatable.reload();
            });
    ...
    var eventsCapture = function() {
        $(".kt-datatable").on("kt-datatable--on-init",
            function() {
                eventsWriter("Datatable init");
            }).on("kt-datatable--on-layout-updated",
            function() {
                eventsWriter("Layout render updated");
            }).on("kt-datatable--on-ajax-done",
            function() {
                eventsWriter("Ajax data successfully updated");
            }).on("kt-datatable--on-ajax-fail",
            function(e, jqXHR) {
                eventsWriter("Ajax error");
            }).on("kt-datatable--on-goto-page",
            function(e, args) {
                eventsWriter("Goto to pagination: " + args.page);
            }).on("kt-datatable--on-update-perpage",
            function(e, args) {
                eventsWriter("Update page size: " + args.perpage);
            }).on("kt-datatable--on-reloaded",
            function(e) {
                eventsWriter("Datatable reloaded");
            }).on("kt-datatable--on-check",
            function(e, args) {
                eventsWriter("Checkbox active: " + args.toString());
            }).on("kt-datatable--on-uncheck",
            function(e, args) {
                eventsWriter("Checkbox inactive: " + args.toString());
            }).on("kt-datatable--on-sort",
            function(e, args) {
