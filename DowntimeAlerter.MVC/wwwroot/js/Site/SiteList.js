...
var SiteList = {
    ...
    DeleteSite: function(id) {
        ...
        Util.Notification.SwallOptionalParameter("warning",
            ...
            true,
            "Cancel",
            SiteList.CallBackDeleteSiteAlert,
            null,
            formData);
    },
...
var columns = [
    ...
    },
    {
        ...
        width: "5%",
        render: function(data, type, full, meta) {
            var returnValue = '<a style="height:18px;" href="javascript:SiteList.EditSiteModal(\'' +
                full.id +
                '\')" class="btn btn-lg btn-clean btn-icon btn-icon-lg" title="Edit">' +
                ' <i class="flaticon2-pen"></i>' +
                "</a>";
            returnValue += '<a style="height:18px;" href="javascript:SiteList.DeleteSite(\'' +
                full.id +
